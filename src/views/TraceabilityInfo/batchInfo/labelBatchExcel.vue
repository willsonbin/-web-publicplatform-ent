...
export default {
  ...
  methods: {
    downTemplate() {
      ...
      axios({
        method: 'POST',
        url: window.config.traceUrl + '/dict/template/download?type=9',
        data: '',
        responseType: 'blob'
      }).then(res => {
        this.loading = false
    ...
    uploadFile() {
      ...
      this.$refs['basicForm'].validate(valid => {
        if (valid) {
          ...
          this.$backend.request(this.$api.traceabilityInfo.batchImport, { file: this.importForm.file.raw }, {}, false, true, param).then(response => {
            this.loading = false
