  ...
  <div>
    <el-form ref="basicForm" :model="basicForm" label-width="120px" :rules="basicFormRules" label-position="top">
      <el-form-item label="激活类型：" prop="activeType ">
        <el-select v-model="basicForm.activeType " placeholder="请选择" @change="hanleChangeType">
          <el-option label="激活码段" value="0" />
          <el-option label="激活码值" value="1" />
          <el-option label="激活批次" value="2" />
        </el-select>
      </el-form-item>
      ...
      <el-form-item v-if="currentType === '1' || currentType ===2" label="追溯码值：" prop="batchCode">
      ...
      <el-form-item label="激活理由：" prop="remark">
...
export default {
  data() {
    return {
      ...
      basicFormRules: {
        activeType: [{ required: true, message: '请选择激活类型', trigger: 'change' }],
        startSegment: [
        ...
        remark: [{ required: true, message: '请输入激活理由', trigger: 'blur' }]
      },
      batchList: [],
      currentType: '0',
      proBatchList: [],
  ...
  methods: {
    ...
    handleSubmit() {
      this.$refs['basicForm'].validate(valid => {
        if (valid) {
          const url = this.$api.basicSetting.getLabelStatusActive
