<template>
  <div>
    <div class="flex-form-search">
      <el-form ref="searchForm" :model="searchForm" class="tabForm">
        ...
        <el-form-item>
          ...
          <el-button v-if="hasPermission('MENU_BATCH_INVALID_BUTTON_BATCH_INVALID')" type="primary" icon="el-icon-remove-outline" @click="batchVoid">批量作废</el-button>
        </el-form-item>
      </el-form>
    </div>
    <el-table
      v-if="hasPermission('MENU_BATCH_INVALID_BUTTON_LIST')"
      ref="multipleTable"
    ...
    >
      ...
      >
        <template slot-scope="scope">
          <span v-if="scope.row.invalidType===0">作废码段</span>
          <span v-if="scope.row.invalidType===1">作废码值</span>
          <span v-if="scope.row.invalidType===2">作废批次</span>
        </template>
      </el-table-column>
      <el-table-column
        prop="productName"
    ...
    <el-dialog
      ...
      width="40%"
    >
      <FormMadal ref="formMadalRef" @refreshList="getLabelStatusPage" />
